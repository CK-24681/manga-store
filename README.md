...
| **2. Servidor Seguro & IA Dinâmica** | Backend configurado (Node.js/Spring) para intermediar chamadas à IA de forma segura sem expor credenciais. | Rota `POST /api/ai/ask` configurada com o SDK oficial `@google/genai` e **Engenharia de Prompt Avançada**, eliminando 100% de respostas mockadas ou estáticas de mangás. A arquitetura conta com diretrizes estritas de persona (livreiro especialista da Mangazon Store), regras negativas contra clichês de IA (*"Com certeza!"*, *"Olá!"*, *"Sobre sua pergunta..."*), formatação limpa e pipeline de sanitização pós-processamento (`sanitizeAndFilterAIResponse`) para garantir respostas factuais, diretas e elegantes. Em conformidade com o Decreto SAC nº 11.034/2022, inclui suporte a protocolo e transferência para atendimento humano. |
